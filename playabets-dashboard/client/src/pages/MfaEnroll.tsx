...
export default function MfaEnroll({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const { data, error: enrollError } = await supabase.auth.mfa.enroll({
        factorType: "totp",
        friendlyName: `playabets-${Date.now()}`,
      });
      ...
      <div className="w-full max-w-sm" style={CARD_BG}>
        <div className="px-8 py-8">
          ...
          <p className="text-xs text-white/40 mb-6">
            Required for all accounts. Scan the QR code below with an authenticator
            app (Google Authenticator, Authy, 1Password) and enter the 6-digit code.
          </p>
              ...
              {secret && (
                <div className="mb-5">
                  ...
                  <div
                    ...
                    style={{ background: "oklch(0.19 0.04 155)", color: "oklch(0.85 0.005 65)" }}
                  >
                    {secret}
                  </div>
                </div>
